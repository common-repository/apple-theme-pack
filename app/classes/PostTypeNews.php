...
class PostTypeNews {
    ...
    */
    ...
    public function __construct( $app )
    {
        $this->app = $app;
        $this->register_type();
        add_filter( 'manage_apple_news_posts_columns', array( $this, 'print_custom_columns' ) );
        add_action( 'manage_apple_news_posts_custom_column', array( $this, 'print_custom_column_values' ) );
    }
    ...
    */
    public function register_type(){
        ...
        $labels   = array( 
            'name'                  => __( 'News', 'apple-theme-pack' ),
            'singular_name'         => __( 'News', 'apple-theme-pack' ),
            'add_new'               => __( 'Add New', 'apple-theme-pack' ),
            'add_new_item'          => __( 'Add New news', 'apple-theme-pack' ),
            'edit_item'             => __( 'Edit news', 'apple-theme-pack' ),
            'new_item'              => __( 'New news', 'apple-theme-pack' ),
            'view_item'             => __( 'View news', 'apple-theme-pack' ),
            'search_items'          => __( 'Search news', 'apple-theme-pack' ),
            'not_found'             => __( 'No news found', 'apple-theme-pack' ),
            'not_found_in_trash'    => __( 'No news found in Trash', 'apple-theme-pack' ),
            'parent_item_colon'     => __( 'Parent news :', 'apple-theme-pack' ),
            'menu_name'             => __( 'News', 'apple-theme-pack' ) ,
        );

        $args = array( 
            'labels'                => $labels,
            'hierarchical'          => true,
            'description'           => 'Collection of news',
            'supports'              => array( 'title', 'editor', 'excerpt', 'thumbnail' ),
            'public'                => true,
            'show_ui'               => true,
            'show_in_menu'          => true,
            'menu_position'         => 25,
            'menu_icon'             => 'dashicons-admin-post',//image or icon
    ...
    */
    ...
    public function register_meta_boxes( $current_post_type ){
        ...
        add_meta_box( 
            esc_attr( 'apple-theme-pack-news-meta' ), // Unique ID
            __( 'News Details', 'apple-theme-pack' ),// Box title
    ...
    public function render_details_box( $post ){
        ...
        ?>
            ...
            <p><input class="widefat" type="text" name="apple_theme_news_link" value="<?php echo esc_url_raw( get_post_meta( $post->ID, 'apple_theme_news_link', true ) ); ?>"/>
            </p>
        <?php
    ...
    */
    ...
    public function save_meta_box( $post_id ){
        ...
        // do validation here
        if( isset( $_POST['apple_theme_news_link'] ) ){
            update_post_meta( $post_id, 'apple_theme_news_link', esc_url_raw( $_POST['apple_theme_news_link'] ) );
        }
    ...
    public function print_custom_columns( $columns ){  
        ...
        $columns[ 'news_id' ]       = __( 'News Id', 'text-domain' );
